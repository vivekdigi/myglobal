...
@section('content')
    ...
    <div class="mt-2 mb-3 d-flex justify-content-between align-items-center flex-wrap">
        ...
        <div class="d-flex align-items-center" style="gap:8px;">
            ...
            <a href="{{ route('admin.secondary.create') }}" class="btn btn-sm btn-success" style="border-radius:50px;">
            ...
            <a href="{{ route('admin.secondary.list') }}" class="btn btn-sm btn-outline-primary" style="border-radius:50px;">
                <i class="fa fa-list mr-1"></i> All 2nd Accounts
            </a>
        </div>
    </div>
                ...
                <div class="card-header py-2">
                    <div class="d-flex align-items-center flex-wrap" style="gap:8px;">
                        ...
                        <div id="bulkBar" class="d-none align-items-center" style="gap:8px;">
                            ...
                            <form id="bulkDeleteForm" action="{{ route('admin.second.bulk-destroy') }}" method="POST"
                        ...
                        <table class="table table-hover table-sm mb-0">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th>Requested At</th>
                                    <th>Status</th>
                                    <th class="text-center">2nd Accounts</th>
                                ...
                                @forelse ($requests as $req)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @php $secCount = $secondaryCountMap[$req->user_id] ?? 0; @endphp
                                            @if ($secCount > 0)
                                                <a href="{{ route('admin.secondary.list', ['search' => $req->user_name]) }}"
                                                   class="badge badge-success px-2 py-1"
                                                   style="font-size:.8rem; border-radius:50px; text-decoration:none;"
                                                   title="View secondary accounts for {{ $req->user_name }}">
                                                    <i class="fa fa-user-circle mr-1"></i>{{ $secCount }}
                                                </a>
                                            @else
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu{{ $req->id }}">
                                                    ...
                                                    @else
                                                        <a href="{{ route('admin.secondary.create', ['user_id' => $req->user_id]) }}" class="dropdown-item text-primary">
                                                            <i class="fa fa-user-plus mr-2"></i> Create 2nd Account
                                                        </a>
                                                    @endif
                                                    <div class="dropdown-divider"></div>
                                                    <form action="{{ route('admin.second.destroy', $req->id) }}" method="POST"
                                                          onsubmit="return confirm('Delete this second account request? This cannot be undone.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="dropdown-item text-danger">
