...
@section('content')
    ...
    <div class="mt-2 mb-3 d-flex justify-content-between align-items-center flex-wrap">
        <div>
            <h4 class="title1 mb-0">Fourth Account Requests</h4>
            <small class="text-muted">Users who have requested a fourth trading account</small>
        </div>
        <div class="d-flex align-items-center" style="gap:8px;">
            <span class="badge badge-primary badge-pill px-3 py-2" style="font-size:.85rem;">
                {{ $requests->total() }} Total
            </span>
            <a href="{{ route('admin.fourth.create') }}" class="btn btn-sm btn-success" style="border-radius:50px;">
                <i class="fa fa-plus mr-1"></i> Create 4th Account
            </a>
            <a href="{{ route('admin.fourth.list') }}" class="btn btn-sm btn-outline-primary" style="border-radius:50px;">
                <i class="fa fa-list mr-1"></i> All 4th Accounts
            </a>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header py-2">
                    <div class="d-flex align-items-center flex-wrap" style="gap:8px;">
                        {{-- Search --}}
                        <form method="GET" action="{{ route('admin.fourth.requests') }}" class="d-flex align-items-center" style="gap:8px;">
                            ...
                            @if($search)
                                <a href="{{ route('admin.fourth.requests') }}" class="btn btn-sm btn-secondary">
                        ...
                        <div id="bulkBar" class="d-none align-items-center" style="gap:8px;">
                            ...
                            <form id="bulkDeleteForm" action="{{ route('admin.fourth.bulk-destroy') }}" method="POST"
                        ...
                        <table class="table table-hover table-sm mb-0">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th>Requested At</th>
                                    <th>Status</th>
                                    <th class="text-center">4th Accounts</th>
                                ...
                                @forelse ($requests as $req)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @php $fCount = $fourthCountMap[$req->user_id] ?? 0; @endphp
                                            @if ($fCount > 0)
                                                <a href="{{ route('admin.fourth.list', ['search' => $req->user_name]) }}"
                                                   class="badge badge-success px-2 py-1"
                                                   style="font-size:.8rem; border-radius:50px; text-decoration:none;"
                                                   title="View 4th accounts for {{ $req->user_name }}">
                                                    <i class="fa fa-user-circle mr-1"></i>{{ $fCount }}
                                                </a>
                                            @else
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu{{ $req->id }}">
                                                    @if ($req->status !== 'approved')
                                                        <form action="{{ route('admin.fourth.status', $req->id) }}" method="POST">
                                                            @csrf
                                                    ...
                                                    @else
                                                        <a href="{{ route('admin.fourth.create', ['user_id' => $req->user_id]) }}" class="dropdown-item text-primary">
                                                            <i class="fa fa-user-plus mr-2"></i> Create 4th Account
                                                        </a>
                                                    @endif
                                                    <div class="dropdown-divider"></div>
                                                    <form action="{{ route('admin.fourth.destroy', $req->id) }}" method="POST"
                                                          onsubmit="return confirm('Delete this fourth account request? This cannot be undone.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="dropdown-item text-danger">
                                    ...
                                    <tr>
                                        <td colspan="9" class="text-center py-5 text-muted">
                                            ...
                                            @else
                                                ...
                                                No fourth account requests yet.
                                            @endif
                                        </td>
                                    </tr>
                                @endforelse
