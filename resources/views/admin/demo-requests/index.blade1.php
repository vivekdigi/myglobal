...
@section('content')
    ...
    <div class="mt-2 mb-3 d-flex justify-content-between align-items-center flex-wrap">
        <div>
            <h4 class="title1 mb-0">Demo Account Requests</h4>
            <small class="text-muted">Users who have requested a demo trading account</small>
        </div>
            ...
            <div class="card">
                <div class="card-header py-2">
                    <div class="d-flex align-items-center flex-wrap" style="gap:8px;">
                        {{-- Search --}}
                        <form method="GET" action="{{ route('admin.demo.requests') }}" class="d-flex align-items-center" style="gap:8px;">
                            ...
                            @if($search)
                                <a href="{{ route('admin.demo.requests') }}" class="btn btn-sm btn-secondary">
                                    <i class="fa fa-times"></i> Clear
                                </a>
                            @endif
                        </form>

                        {{-- Bulk action bar — shown only when checkboxes are ticked --}}
                        <div id="bulkBar" class="d-none align-items-center" style="gap:8px;">
                            ...
                            <form id="bulkDeleteForm" action="{{ route('admin.demo.bulk-destroy') }}" method="POST"
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover table-sm mb-0" id="demoTable">
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu{{ $req->id }}">
                                                    @if ($req->status !== 'approved')
                                                        <form action="{{ route('admin.demo.status', $req->id) }}" method="POST">
                                                            @csrf
                                                    ...
                                                    <form action="{{ route('admin.demo.destroy', $req->id) }}" method="POST"
                                                          onsubmit="return confirm('Delete this demo request? This cannot be undone.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="dropdown-item text-danger">
...
(function () {
    ...
    window.confirmBulk = function (form) {
        ...
        if (!confirm('Delete ' + checked.length + ' selected record(s)? This cannot be undone.')) return false;
