...
class ManageUsersController extends Controller
{
    ...
    public function showUsers($id)
    {
        ...
        $hasRef = request()->query('usersHaveRef', true);

        if ($hasRef) {
            $ref = User::where('id', '!=', $id)->select(['id', 'name'])->get();
        } else {
            $ref = User::whereNull('ref_by')->where('id', '!=', $id)->select(['id', 'name'])->get();
        }
    ...
    {
        $users = User::select('name', 'id', 'email')->orderByDesc('id')->get();
    ...
    public function viewuser($id)
    {
        ...
        // If this is a secondary account → fetch parent
        $parentUser = null;
        if ($user->is_secondary && $user->parent_user_id) {
            $parentUser = User::find($user->parent_user_id);
        }

        // If this is a primary account → fetch secondary account if exists
        $secondaryUser = null;
        if (!$user->is_secondary) {
            $secondaryUser = User::where('parent_user_id', $user->id)
